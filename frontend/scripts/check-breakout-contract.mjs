...
const checks = [
  ...
  ["Breakout run-scan IBKR branch", files.breakout, 'if (dataSource !== "yfinance")'],
  ["Breakout run-scan IBKR call", files.breakout, "return loadIbkrScan(config.custom_symbols"],
  ["Breakout IBKR interval parameter", files.breakout, "interval"],
  ["Breakout IBKR refresh parameter", files.breakout, "refresh"],
  ...
  ["Breakout IBKR intraday tab label", files.breakout, "IBKR Intraday · Nasdaq-100 30m"],
  ...
  ["Wheel quotes endpoint", files.wheel, "/api/quotes?"],
  ["Wheel quotes symbols parameter", files.wheel, "symbols: symbols.join"],
